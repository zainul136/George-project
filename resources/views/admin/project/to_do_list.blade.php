...
<body class="  ">
    ...
    <main class="main-content">
        <div class="position-relative iq-banner">
            ...
            <div class="iq-navbar-header"></div>
            <!--Nav End-->
        </div>
                            ...
                            <div class="card-body">
                                <div class="row mb-2">
                                    <div class="col-md-6 text-start">
                                        <h4 class="card-title">All Events Details</h4>
                                    </div>
                                </div>
                                <div class="table-responsive">
                                    <table id="eventsTable" class="table table-bordered" style="cursor: all-scroll;">
                                        <thead>
                                            <tr>
                                                <th scope="col" class="text-uppercase">Project Name</th>
                                                <th scope="col" class="text-uppercase"> Event Name</th>
                                            ...
                                            @foreach ($events as $event)
                                                ...
                                                <tr class="draggable-row" id="{{ $event->id }}">
                                                    ...
                                                    <td data-date="{{ $eventDate }}"
                                                        class=" @if ($event->action_date < date('Y-m-d', strtotime(now())) && $event->status !== 'completed') text-danger font-weight-bold @endif">
                                                        {{ $eventDate }}
                                                    </td>
                                                    <td
                                                        class=" @if ($event->old_date < date('Y-m-d', strtotime(now())) && $event->status !== 'completed') text-danger font-weight-bold @endif">
                                                        {{ $event->old_date ?? '' }}
                                                    </td>
            ...
            $('.edit_event_').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        modal.find('.project_id').val(response.project_id);
